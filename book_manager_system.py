...
class Book_Manager:
        ...
        try:
            ...
            if result:
                ...
                if len(account)>=4 and len(password)>=4:
                    ...
                    self.conn.commit()
                    print("注册成功！")
                    self.write_log(f"用户{account}注册")
                else:
                    print("请重新输入！账号密码至少4位")
        except Exception as e:
            self.conn.rollback()
    ...
    def alter_pwd(self,account,password):
        try:
            if account=='admin':
                print("无法修改此账号密码")
            else:
    ...
    def borrow_msg(self,borrow_time,return_time,account):
        try:
            sql="select count(*) from borrower where account=%s"
            self.cursor.execute(sql,(account,))
            count = self.cursor.list(self.cursor.fetchone().values())[0]

            sql="insert into borrower (account,borrow_time,return_time,count) values(%s,%s,%s,%s)"
            self.cursor.execute(sql,(account,borrow_time,return_time,count))
            self.conn.commit()

            print("借阅信息记录成功！")
            self.write_log(f" 借阅人:{account}  借阅时间:{borrow_time}  应归还时间:{return_time}  总借阅数量:{count}")
        except Exception as e:
            print("记录失败!")
            self.conn.rollback()
    ...
    def fuzzy_select(self,book_name):
        ...
        if not result:
            print("查询不到该图书信息,请重新查询")
            return
        self.write_log(f"模糊查询   图书编号:{result['book_id']}  书名:《{result['book_name']}》")
        print(f"=======图书信息如下=======")
        print(f"图书编号：{result['book_id']}")
        print(f"书名：《{result['book_name']}》")
        print(f"作者：{result['author']}")
    ...
    def category_select(self,category):
        ...
        if not result:
            print("该分类不存在图书")
            return
        self.write_log(f"分类查询   图书分类:{category}")
        print(f"=======图书信息如下=======")
        for row in result:
            print(f"图书编号:{row['book_id']}     书名:《{row['book_name']}》     作者:{row['author']}     图书分类:{row['category']}     图书状态:{row['status']}")
    ...
    def author_select(self,author):
        ...
        if not result:
            print("不存在该作者")
            return
        self.write_log(f"按作者查询     作者名:{author}")
        print(f"=======图书信息如下=======")
        for row in result:
            print(
